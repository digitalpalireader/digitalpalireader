  ...
  else
  {
    ...
    for(var i=0; i < bNodes.length; i++)
    {
      ...
      if(loc[7] == 'm') {
        ...
        if(sname) sname = ' ('+glblObj.G_nikLongName[loc[0]]+' '+sname+')';
        else sname = '';
      }
      else var sname = ' ('+glblObj.G_nikLongName[loc[0]]+')';
      loc[0] = "'"+loc[0]+"'";
      loc[7] = "'"+loc[7]+"'";
...
function convertOldBookmarks() {
  ...
  for(var i=0;i < ca.length;i++)
  {
    if (/^DPB/.exec(ca[i])) {
      ...
      ba[name]['loc'] = ba[name]['loc'].replace(/^([0-9]+)/,glblObj.G_numberToNik[nik]);
