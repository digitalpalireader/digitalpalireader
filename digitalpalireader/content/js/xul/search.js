...
function DPR_PAL_Search_AddSectionLink(nik) {
  ...
  var setlabel = MD.createElement('label');
  setlabel.setAttribute('value', glblObj.G_nikLongName[nik] + ': 0');
...
function DPR_PAL_Search_AddSearchTermSectionInfo(sectionInfo) {
  ...
  thisterm.setAttribute('value',(glblObj.G_searchRX?glblObj.G_searchString:toUni(glblObj.G_searchString))+': ');
  thisterm.setAttribute('id','search-term');
...
function DPR_PAL_Search_AddCopyPermaLinkElement() {
  ...
  mlink.setAttribute('onmouseup','permalinkClick(\''+glblObj.G_searchLink+'\',1)');
...
function DPR_PAL_Search_CreateSectionHeader(newnikaya) {
  ...
  headingNode.innerHTML = glblObj.G_nikLongName[newnikaya] + '<hr>';
  return headingNode;
}
