...
var DPRNav = {
  changeSet:function(noget,book){
    ...
    if (glblObj.G_hier == 't' && this.limitt(document.getElementById('set').selectedIndex)) {
      alert('Ṭīkā not available for '+glblObj.G_nikLongName[document.getElementById('set').value]+'.');
      document.getElementById('set').selectedIndex = oldnikaya;
      return;
    }
    if (glblObj.G_hier == 'a' && document.getElementById('set').value == 'g') {
      alert('Atthakatha not available for Gram.');
      document.getElementById('set').selectedIndex = oldnikaya;
      return;
    }
    if (glblObj.G_hier == 'a' && document.getElementById('set').value == 'b') {
  ...
  setSearchBookList:function() {
    ...
    for (var i = 0; i < titles.length; i++) {
      ...
      // menu
      bookNode.appendItem(translit((nik == 'k' || nik == 'y' || nik == 'n') ? glblObj.G_kynames[nik][titles[i]] : glblObj.G_nikLongName[nik] + ' ' + titles[i]),((nik == 'k' || nik == 'y' || nik == 'n') ? (titles[i]+1) : (i+1)));
      ...
      newCheck.setAttribute('label',((nik == 'k' || nik == 'y' || nik == 'n') ? glblObj.G_kynames[nik][titles[i]] : (typeof(titles[i]) == 'number' ? 'Book ' : '') + titles[i]));
  ...
  switchhier:function(htmp) {
    ...
    if (htmp == 'a' && document.getElementById('set').selectedIndex > 7) {
      alert('Aṭṭhakathā not available for ' + glblObj.G_nikLongName[document.getElementById('set').value]+'.');
  ...
  gotoPlace:function([nikaya,book,meta,volume,vagga,sutta,section,hiert]) {
    ...
    for(var i in glblObj.G_hshort) {
      if(i == hiert)
        document.getElementById(glblObj.G_hshort[i]).setAttribute('checked',true);
      else
        document.getElementById(glblObj.G_hshort[i]).checked = false;
    }
    this.changeSet(1,book);
