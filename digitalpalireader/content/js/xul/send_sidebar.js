...
var DPRSend = {
  ...
  importXML:function(section,labelsearch,para,isPL,add,scroll,cat) {
    ...
    if(!add) { // reuse old tab
      ...
      if(thisTab) {
        ...
        thisTabBrowser.contentDocument.getElementById('dpr-tops').getElementsByTagName('browser')[0].contentWindow.loadXMLSection(labelsearch,para,[nikaya,bookno,meta,volume,vagga,sutta,section,DPR_G.G_hier]);
        return;
      }
      ...
      if (!oldTab) {
        ...
        oldTabBrowser.contentDocument.getElementById('dpr-tops').getElementsByTagName('browser')[0].contentWindow.loadXMLSection(labelsearch,para,[nikaya,bookno,meta,volume,vagga,sutta,section,DPR_G.G_hier]);
      }
    }
    else if (add == 'shift') {
      var thisTab = DPRChrome.isThisDPRTab('DPRm');
  ...
  importXMLindex:function(add) {
    ...
    if(!add) { // reuse old tab
      ...
      if(thisTab) {
        var thisTabBrowser = mainWindow.gBrowser.getBrowserForTab(thisTab);
        thisTabBrowser.contentDocument.getElementById('dpr-tops').getElementsByTagName('browser')[0].contentWindow.loadXMLindex([nikaya,bookno,DPR_G.G_hier]);
        return;
      }
      var oldTab = DPRChrome.findDPRTab('DPR-main');
      ...
      if (!oldTab) {
        ...
        mainWindow.gBrowser.selectedTab = oldTab;
        var oldTabBrowser = mainWindow.gBrowser.getBrowserForTab(oldTab);
        oldTabBrowser.contentDocument.getElementById('dpr-tops').getElementsByTagName('browser')[0].contentWindow.loadXMLindex([nikaya,bookno,DPR_G.G_hier]);
      }
    }
    else if (add == 'shift') {
      var thisTab = DPRChrome.isThisDPRTab('DPRm');
  ...
  sendDict: function (hard,add,which,getstring,opts) {
    ...
    if(!getstring) {
      ...
      if(!hard) {
        if (getstring == DPR_G.G_lastsearch || getstring == '' || !DPR_prefs['autodict'] || document.getElementById('soregexp').checked || document.getElementById('sofulltext').checked) return;
      }
