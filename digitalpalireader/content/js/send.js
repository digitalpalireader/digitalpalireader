...
function importXMLindex(add) {
  ...
  if(!add) { // reuse old tab
    ...
    openXMLindex(nikaya,bookno,glblObj.G_hier,add);
  }
}
...
function sendAnalysisToOutput(input, divclicked, frombox, add){
  ...
  if(divclicked) {
    ...
    if (cdiv)
    {
      var ldiv = document.getElementById(glblObj.G_lastcolour);
      if (ldiv)
      {
      ...
      glblObj.G_lastcolour = divclicked;
    }
