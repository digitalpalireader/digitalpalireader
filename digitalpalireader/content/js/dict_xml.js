...
function paliXML (filein, which, add) {
          ...
          datat +=
            dataa[i]
              ...
              .replace(
                ...
                ' <a style="color:' +
                  DPR_prefs['colsel'] +
                  '" href="javascript:void(0)" onclick="paliXML(\'PED/' +
                  P[tda][0] +
                  ',' +
                  toUni(tda) +
                  '\')">' +
  ...
  if (G_pedhist.length > 1) {
    // show select
    var showing =
      '<select title="go to history" onchange="if(this.selectedIndex != 0) { G_phmark=this.length-1-this.selectedIndex; paliXML(this.options[this.selectedIndex].value,1);}"><option>- history -</option>'
  ...
  if (lname)
    tout +=
      '<span class="abut lbut tiny" onclick="paliXML(\'PED/' +
      lname +
      '\')" />&lt;</span>'
  if (nname)
    tout +=
      '<span class="abut rbut tiny" onclick="paliXML(\'PED/' +
      nname +
      '\')" />&gt;</span>'
...
function DPPNXML (filein, which, add) {
  ...
  if (!which) {
    ...
    for (i in G_dppnhist) {
      if (G_dppnhist[i] != file) {
        dppnhistt.push(G_dppnhist[i])
      }
    }
  ...
  if (G_dppnhist.length > 1) {
    // show select
    var showing =
      '<select title="go to history" onchange="if(this.selectedIndex != 0) { G_dhmark=this.length-1-this.selectedIndex; DPPNXML(this.options[this.selectedIndex].value,1);}"><option>- history -</option>'
  ...
  if (lname)
    tout +=
      '<span class="abut lbut tiny" onclick="DPPNXML(' +
      lname +
      ')" />&lt;</span>'
  if (nname)
    tout +=
      '<span class="abut rbut tiny" onclick="DPPNXML(' +
      nname +
      ')" />&gt;</span>'
...
function outputSktRoots (xmlhttpResponseXML) {
  ...
  data = data
    ...
    .replace(
      /<a href="([^.]+)[^>]+/g,
      '<a class="green" href="javascript:void(0)" onclick="sktRXML(\'$1\')"'
    )
...
function getAtthXML (num, type, niklist) {
  ...
  location: for (i in loca) {
    ...
    finout +=
      '<p><span class="abut obut tiny" onclick="openPlace([\'' +
      nikaya +
      "'," +
      bookno +
      ',' +
...
function getTitleXML (num, mul, att, tik, niklist) {
  ...
  location: for (i in loca) {
    ...
    var sn =
      hiert == 'm'
        ...
        : null
    var placen =
      convtitle(nikaya, book, una, vna, wna, xna, yna, zna, hiert, 1)[0] +
      (sn ? ' (' + G_nikLongName[nikaya] + ' ' + sn + ')' : '')

    finout +=
      '<p>' +
      placen +
      ' <span class="abut obut" onclick="openPlace([\'' +
      nikaya +
      "','" +
      bookno +
      "','" +
...
function getDppnData (link) {
  ...
  var data =
    ...
    xmlDoc
      .getElementsByTagName('e')
      [parseInt(link.split('/')[1])].textContent.replace(/\[/g, '<')
      .replace(/\]/g, '>')
      .replace(/href/g, 'style="color:blue" href')
      .replace(/\.  /g, '.&nbsp; ')
  return data
