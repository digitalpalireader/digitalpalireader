...
function DPR_PAL_Search_AddSectionLink(nik) {
  ...
  const html = `
<li>
  <a id="${id}" href="#" onclick="return scrollSearch('${scrollTo}')">${glblObj.G_nikLongName[nik] + ': 0'}</a>
</li>`;
  MD.getElementById('search-sets').insertAdjacentHTML('beforeend', html);
}
...
function DPR_PAL_Search_AddSearchTermSectionInfo(sectionInfo) {
  const html = `
<li>
  <label class="m-0" id="search-term" style="font-weight: bold;">${(glblObj.G_searchRX?glblObj.G_searchString:toUni(glblObj.G_searchString))+': '}</label>
</li>
<li>
...
function DPR_PAL_Search_AddCopyPermaLinkElement() {
  const html = `
<a class="btn btn-success btn-light btn-small m-0" onclick="permalinkClick('${glblObj.G_searchLink}')" title="Click to copy permalink to clipboard"><i class="fa fa-link" aria-hidden="true"></i></a>
`;
...
function DPR_PAL_Search_CreateSectionHeader(newnikaya) {
  ...
  const html = `<h4 id='${id}' name='xyz' class="card-title huge">${glblObj.G_nikLongName[newnikaya]}</h4>`;
