...
var DPRSend = {
  ...
  importXML: function (section, labelsearch, para, isPL, add, scroll, cat) {
    ...
    const loadXmlSectionParams = [nikaya, bookno, meta, volume, vagga, sutta, section, glblObj.G_hier];
  ...
  importXMLindex: function (add) {
    ...
    const loadXmlIndexParams = [nikaya, bookno, glblObj.G_hier];
  ...
  sendDict: function (hard, add, which, getstring, opts) {
    ...
    if (!getstring) {
      ...
      if (!hard) {
        if (getstring == glblObj.G_lastsearch || getstring == '' || !DPR_prefs['autodict'] || $('#soregexp').prop("checked") || $('#sofulltext').prop("checked")) return;
      }
