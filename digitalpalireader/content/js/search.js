...
glblObj.G_searchStartTime;

glblObj.G_searchType;
glblObj.G_searchString;
glblObj.G_searchMAT;
glblObj.G_searchSet;
glblObj.G_searchBook;
glblObj.G_searchPart;
glblObj.G_searchRX;
glblObj.G_searchLink;
...
function searchTipitaka(searchType,searchString,searchMAT,searchSet,searchBook,searchPart,searchRX) {
  ...
  if(searchString) { // update url
    glblObj.G_searchType = searchType;
    glblObj.G_searchString = searchString;
    glblObj.G_searchMAT = searchMAT;
    glblObj.G_searchSet = searchSet;
    glblObj.G_searchBook = ','+searchBook+',';
    glblObj.G_searchPart = searchPart;
    glblObj.G_searchRX = searchRX;
    ...
    for (var i in options) {
      ...
      switch(option[0]) {
        case 'type':
          glblObj.G_searchType = parseInt(option[1]);
        break;
        case 'query':
          glblObj.G_searchString = decodeURIComponent(option[1]);
        break;
        case 'MAT':
          glblObj.G_searchMAT = option[1];
        break;
        case 'set':
          glblObj.G_searchSet = option[1];
        break;
        case 'book':
          glblObj.G_searchBook = ','+option[1]+','; // add commas so each will be findable with comma at end and beginning
        break;
        case 'part':
          glblObj.G_searchPart = option[1];
        break;
        case 'rx':
          glblObj.G_searchRX = (option[1] == 'true');
        break;
      }
    }
  }

  if(!glblObj.G_searchString) {
    var check = {value: false};                  // default the checkbox to false
    var input = {value: ''};
    var result = glblObj.G_prompts.prompt(null, "Enter Query", "Enter query for custom search:", input, null, check);
    if(!result)
      return;
    if(!input.value)
      return;
    glblObj.G_searchString = input.value;
  }

  var st = [];
  var matst = [];
  for (var i in glblObj.G_searchMAT) matst.push(glblObj.G_searchMAT[i]);

  st[0] = 'Multiple Sets';
  st[1] = glblObj.G_nikLongName[glblObj.G_searchSet] + ' Books';
  st[2] = glblObj.G_nikLongName[glblObj.G_searchSet] + ' ' + glblObj.G_searchBook.slice(1,-1);
  st[3] = glblObj.G_nikLongName[glblObj.G_searchSet] + ' ' + glblObj.G_searchBook.slice(1,-1) + ' (partial)';
  st[5] = 'ATI Translations';

  if(glblObj.G_searchType == 0 && /\|/.test(glblObj.G_searchBook)) // custom search
    st[0]+=' (custom)';


  // tab title

  var tabT = 'Search: \'' + (glblObj.G_searchRX?toUniRegEx(glblObj.G_searchString):toUni(glblObj.G_searchString)) + '\' in ' + st[glblObj.G_searchType];

  DPR_PAL_Search_SetTitle(tabT);

  if (/^[TPVMtpvm][0-9]\.[0-9][0-9][0-9][0-9]$/.exec(glblObj.G_searchString)) {  // page search
    glblObj.G_searchString = glblObj.G_searchString.toUpperCase();
  }
  else if(glblObj.G_searchType != 5) {
    glblObj.G_searchString = glblObj.G_searchString.toLowerCase();
  }


  switch(glblObj.G_searchType) {
    case 0:
      pausesall();
    break;
    case 1:
...
function finishSearch() {
  ...
  const searchLink = 'dpr:search?type='+glblObj.G_searchType+'&query=' + toVel(glblObj.G_searchString) + '&MAT=' + glblObj.G_searchMAT + '&set=' + glblObj.G_searchSet + '&book=' + glblObj.G_searchBook.slice(1,-1) + '&part=' + glblObj.G_searchPart + '&rx=' + glblObj.G_searchRX;
  glblObj.G_searchLink = DPR_PAL.normalizeDprUri(searchLink);
...
function pausesall()
{
  // make glblObj.G_searchFileArray
  var which = glblObj.G_searchType;

  if(/\|/.test(glblObj.G_searchBook)) { // custom search
    glblObj.G_searchBook = glblObj.G_searchBook.replace(/\|/g,',|,');
    var cbooks = glblObj.G_searchBook.split('|');
    var cniks = [];
    for(var i in glblObj.G_searchSet) {
      cniks[glblObj.G_searchSet[i]] = i;
    }
  }

  for(var w in glblObj.G_XMLFileArray) {
    if (glblObj.G_searchSet.indexOf(w.charAt(0)) == -1) continue; // don't add unchecked collections

    for (var x = 0; x < 3; x++) {
      if(glblObj.G_searchMAT.indexOf(glblObj.G_hLetters[x]) > -1 && glblObj.G_XMLFileArray[w][x] == 1) { // this hier is checked and the file exists in this hier
        ...
        glblObj.G_searchFileArray.push(w+glblObj.G_hLetters[x]);
      }
    }
  }

  if(glblObj.G_searchFileArray.length == 0) {
    alert('No books in selection');
    return;
  }

  var getstring = glblObj.G_searchString;

  DPR_PAL_Search_ClearSearchResults();

  DPR_PAL_SearchAddSearchTermSectionLink(glblObj.G_searchRX ? glblObj.G_searchString : toUni(glblObj.G_searchString));

  for (var i = 0; i < glblObj.G_numberToNik.length; i++) {
    if (glblObj.G_searchSet.indexOf(glblObj.G_numberToNik[i]) == -1) continue; // don't add unchecked collections
    DPR_PAL_Search_AddSectionLink(glblObj.G_numberToNik[i]);
  }

  DPR_PAL_Search_MakeProgressTable(glblObj.G_searchFileArray.length - 1);
...
function pausetwo() { // init function for single collection
  ...
  // make glblObj.G_searchFileArray
  var which = glblObj.G_searchType;
  var nikaya = glblObj.G_searchSet;

  for(var w in glblObj.G_XMLFileArray) {
    if (w.charAt(0) != nikaya) continue; // only this collection
    if (glblObj.G_searchBook.indexOf(','+w.substring(1)+',') == -1) continue; // skip unchecked books
  ...
  DPR_PAL_Search_AddSearchTermSectionInfo(glblObj.G_nikLongName[nikaya]);
...
function pausethree() {
  ...
  var which = glblObj.G_searchType;
  var nikaya = glblObj.G_searchSet;
  var book = glblObj.G_searchBook.slice(1,-1);

  var nikbook = nikaya+book;
  var getstring = glblObj.G_searchString;
  if(which == 2) { // single book, multiple hier
    for (var x = 0; x < 3; x++) {
      if(glblObj.G_searchMAT.indexOf(glblObj.G_hLetters[x]) > -1 && glblObj.G_XMLFileArray[nikbook][x] == 1) { // this hier is checked and the file exists in this hier
        glblObj.G_searchFileArray.push(nikbook+glblObj.G_hLetters[x]);
      }
    }
  }
  else { // single book (partial)
    glblObj.G_searchFileArray = [nikbook+glblObj.G_searchMAT];
  }
  ...
  DPR_PAL_Search_AddSearchTermSectionInfo(glblObj.G_nikLongName[nikaya]+' '+book + (glblObj.G_searchPart != '1'?' (partial)':''));
...
function importXMLs(cnt)
{
  ...
  {
    bookfile = glblObj.G_searchFileArray[qz];
    ...
    if (qz < glblObj.G_searchFileArray.length-1)
    {
      nextbookfile = glblObj.G_searchFileArray[qz+1];
...
function createTables(xmlDoc,hiert)
{
  ...
  if (yesplus >= 0) {
    stringra = (glblObj.G_searchRX?toUniRegEx(getstring):toUni(getstring)).split('+');
  }
  else {
    stringra[0] = getstring;
    if(glblObj.G_searchRX) getstring = toUniRegEx(getstring);
    else getstring = toUni(getstring);
  }
  var sraout = stringra.join('#');
  sraout = sraout.replace(/"/g, '`');
  //if(glblObj.G_searchRX) sraout = '/'+sraout.replace(/\\/g,'\\\\')+'/';

  var part;
  if(glblObj.G_searchPart != '1') part = glblObj.G_searchPart.split('.');

  for (var sx = 0; sx < u.length; sx++) // per h0
  {

    if(glblObj.G_searchType == 3 && part[0] >= 0 && sx != parseInt(part[1])) continue;
    ...
    for (var sy = 0; sy < v.length; sy++) // per h1
    {
      ...
      if(glblObj.G_searchType == 3 && part[0] >= 1 && sy != parseInt(part[2])) continue;
      ...
      for (var sz = 0; sz < w.length; sz++) // per h2
      {
        ...
        if(glblObj.G_searchType == 3 && part[0] >= 2 && sz != parseInt(part[3])) continue;
        ...
        for (var s = 0; s < x.length; s++) // per h3
        {
          ...
          if(glblObj.G_searchType == 3 && part[0] >= 3 && s != parseInt(part[4])) continue;
          ...
          for (var se = 0; se < y.length; se++) // per h4
          {
            ...
            if(glblObj.G_searchType == 3 && part[0] >= 4 && se != parseInt(part[5])) continue;
            ...
            for (var tmp = 0; tmp < z.length; tmp++) // per paragraph
            {
              ...
              if (!/[0-9]/.exec(glblObj.G_searchString)) texttomatch = texttomatch.replace(/\^a\^[^^]*\^ea\^/g, ''); // remove pesky page references unless we're searching for them.
              ...
              if (yesplus >= 0) // multiple search terms
              {
                ...
                for (var d = 0; d < stringra.length; d++)
                {
                  ...
                  if (startmatch >= 0)
                  {
                    ...
                    while (startmatch >= 0)
                    {
                      if(glblObj.G_searchRX) gotstring = texttomatch.match(perstring)[0];
                      ...
                      if(glblObj.G_searchRX) startmatch = findRegEx(texttomatch,perstring);
                ...
                if (yesall == stringra.length)
                {
                  for(var t=0; t<tempexword.length; t++) {
                    ...
                    for(var i=0; i<l; i++) {
                      if (/^[TPVM][0-9]\.[0-9][0-9][0-9][0-9]$/.exec(getstring)) { // page search
                        ...
                        while (!glblObj.G_uniRegExp.exec(tempexword[t][i].charAt(0)) && !/[0-9]/.exec(tempexword[t][i].charAt(0))) {
                          tempexword[t][i] = tempexword[t][i].substring(1);
                        }
                        while (!glblObj.G_uniRegExp.exec(tempexword[t][i].charAt(tempexword[t][i].length-1)) && !/[0-9]/.exec(tempexword[t][i].charAt(tempexword[t][i].length-1))) {
                          tempexword[t][i] = tempexword[t][i].slice(0,-1);
                        }
                      }
                    }
                  ...
                  finalout += '<div id='+countmatch + tagtitle+'><p><span><b style="color:' + DPR_prefs['colsel'] + '">' + glblObj.G_nikLongName[nikaya] + (hiert == 'm' ? '' : '-'+hiert) + ' ' + bookname + '</b>';
                  var colt = 0;
              ...
              else // single search term
              {
                ...
                if(glblObj.G_searchRX) startmatch = findRegEx(texttomatch,getstring);
                ...
                if (startmatch >= 0)
                {
                  ...
                  {
                    match = 1;
                                        if(glblObj.G_searchRX) gotstring = texttomatch.match(getstring)[0];
                  ...
                  if (/^[TPVM][0-9]\.[0-9][0-9][0-9][0-9]$/.exec(getstring)) { // not page search
                    ...
                    for(var i=0; i<l; i++) {
                      ...
                      while (!glblObj.G_uniRegExp.exec(tempexword[i].charAt(0)) && !/[0-9]/.exec(tempexword[i].charAt(0))) {
                        tempexword[i] = tempexword[i].substring(1);
                      }
                      while (!glblObj.G_uniRegExp.exec(tempexword[i].charAt(tempexword[i].length-1)) && !/[0-9]/.exec(tempexword[i].charAt(tempexword[i].length-1))) {
                        tempexword[i] = tempexword[i].slice(0,-1);
                      }
                    }
                  }
                  ...
                  if(hiert == 'm') {
                    ...
                    finalout +=  (modno ? ' (<b class="small" style="color:'+DPR_prefs['colsel']+'">' + glblObj.G_nikLongName[nikaya] + (hiert == 'm' ? '' : '-'+hiert) + '&nbsp;' + modno + '</b>)' : '');
                  }
...
//    $('#stfb').html('<table><tr id="atiNiks"><td width=1><a href="javascript:void(0)" onclick="this.blur(); stopsearch = 1" title="click to stop search"><img id="stfstop" src="images/stop.png" width=25></a></td><td><a href="http://www.accesstoinsight.org" title="Access To Insight Website"><img src="'+atiurl+'favicon.ico"> ATI</a> full-text search for <b style="color:'+DPR_prefs['colped']+'">'+getstring+'</b> (off-line): </td></tr></table>');
    DPR_PAL_Search_MakeProgressTable(glblObj.G_searchSet.length);
    ...
    setlabel.setAttribute('value',(glblObj.G_searchRX?glblObj.G_searchString:toUni(glblObj.G_searchString)));
    ...
    for (var i = 0; i < glblObj.G_numberToNik.length; i++) {
      ...
      thisset.setAttribute('onmouseup','scrollSearch(\'sbfN'+glblObj.G_numberToNik[i]+'\')');

      var setlabel = MD.createElement('label');
      setlabel.setAttribute('value',glblObj.G_nikLongName[glblObj.G_numberToNik[i]]+': 0');
      setlabel.setAttribute('id','matches'+glblObj.G_numberToNik[i]);
      setlabel.setAttribute('class','search-button-label');
      ...
      if(i < glblObj.G_searchSet.length)
...
function atiSearchOffline(d, getstring) {
  ...
  var nikA = ['d','m','s','a','k'];
  while (glblObj.G_searchSet.indexOf(nikA[d]) == -1) {
    d++;
  ...
  for (var c = 0; c < anik.length; c++) {
    ...
    for (var j in data) {
      ...
      var texttomatch = data[j].textContent;
        if(glblObj.G_searchRX) startmatch = texttomatch.search(getstring);
        else startmatch = texttomatch.indexOf(getstring)
        postpara = '';
        if (startmatch >= 0)
        {
  ...
  cellNode.innerHTML = '<a class="green" href="javascript:void(0)"'+(count > 0 ? ' onclick="scrollToId(\'dif\',\'atiL'+nik+'\')"' : '')+'>'+glblObj.G_nikLongName[nik] + '</a>: ' + count + '; ';
