...
async function sendTextPad(sectionId,add){
  ...
  if(!add) { // reuse old tab
    ...
    if (thisTab) {
      ...
      await thisTabBrowser.contentDocument.getElementById('dpr-tops').getElementsByTagName('browser')[0].contentWindow.analyzeTextPad(sectionId,input);
      return;
    }
    ...
    if (!oldTab) {
      ...
      await oldTabBrowser.contentDocument.getElementById('dpr-tops').getElementsByTagName('browser')[0].contentWindow.analyzeTextPad(sectionId,input);
    }
  } else {
    var permalink = DPR_PAL.toWebUrl('chrome://digitalpalireader/content/index.xul?text='+DPR_translit_mod.toVel(input));
