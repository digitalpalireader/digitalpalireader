            ...
            for (var tmp = 0; tmp < z.length; tmp++) // per paragraph
            {
              ...
              else // single search term
              {
                ...
                if (startmatch >= 0)
                {
                  ...
                  // paragraph
                  finalout += ', para. ' + (tmp + 1) + ' <span class="abut obut" onmouseup="DPR1_send_mod.openPlace(' + `${sectionId}, ` + '[\''+nikaya+'\',' + (book - 1) + ',' + sx + ',' + sy + ',' + sz + ',' + s + ',' + se + ',\''+hiert+'\'],' + (tmp+1) + ',\'' + sraout + '\',DPR1_send_mod.eventSend(event,1))">&rArr;</span></span></p><p>' + DPR1_format_mod.preparepali(sectionId,postpara,3)[0] + '</p><hr></div>';
