	...
	if(cfg['showPedLinks'] == 'checked') {
		...
		for (i = 0; i < dataa.length; i++) {
			if(/<[^>]*$/.exec(dataa[i])) { // pesky broken up links
				...
				else if(typeof(mainda[tda]) == 'object' && tda != toVel(ttit)) datat += dataa[i].replace(/<[^>]*$/,'').replace(toUni(tda), ' <a style="color:'+colorcfg['colsel']+'" href="javascript:void(0)" onclick="paliXML(\'PED/' + mainda[tda][0] + ','+toUni(tda)+'\')">'+toUni(tda)+'</a>') + dataa[i].substring(dataa[i].indexOf(/<[^>]*$/));
				else datat += ' ' + dataa[i];
				i++
...
function getAtthXML(num,type,niklist) { // get atthakatha or tika word 
    ...
    document.getElementById('mafbc').innerHTML = '<b style="text-size:'+(parseInt(colorcfg['colsize'])*2)+'px">'+toUni(word)+'</b> in the '+G_hLetters[type];
