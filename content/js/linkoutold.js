...
// create option dropdown
	...
	for (var b = 0; b < tr.length; b++)
	{
			...
			else if (outwords[b] == 'newrow')
			{
				...
				onetword.push(outwords[b]);
				if (outwords[b].split('^')[1].length > 1) { showword.push(outwords[b].split('^')[1]) }
...
// display first output
	...
	{		
		if(outwords[a])
		{
			...
			else if (outwords[a] == 'out')
			{
				osout += '</i> )';
				outwords[a] = '0^out^0^0';
				//alert('out');
				lastword = 1;
			}
			else if (outwords[a] == 'newrow')
			{
				osout += '</td><td>&nbsp;<br>&nbsp;</td></tr></table><br><table cellspacing="0" cellpadding="0"><tr><td width="1" valign="top" align="center"><i>';
				outwords[a] = '0^newrow^0^0';
				checksum1++;
			}
			else
			{
			//if (a == 0) document.getElementById('debug').innerHTML= '<p>' + outwords[a];
				
				var os = outwords[a].split('^');
				
	
				var paliword = new PaliWord(os[0],os[1],os[2],os[3]);
				
				// os[0] = reference
				// os[1] = pali word
				// os[2] = category
				// os[3] = concise definition (if any)
				// for os[2]: 0 = main, 1 = name, 2 = concise
				...
				if (os[2] != 1 && (os[1].charAt(os[1].length-1) != '-' || os[1].length == 1) && os[2] != 3)
				{
					...
					var oss = new Array();
					if (a > 0) oss = outwords[a-1].split('^');
					var osss = new Array();
					if (outwords[a+1]) osss = outwords[a+1].split('^');
						...
						if (paliword.category != 2)  // ---------- don't bother for nonword entries  ----------
						{
							...
							if (paliword.category != 1) // not a subnote, either a non-match or a upper match
							{
							   ...
							   /* add punctuation
								   ...
								   {
										trpunc = outwords[lookpunc].split('^');
